...
input_csv = r'words_for_translation.csv'
output_csv = r'translation_babla.csv'
...
def scrape_page(word):
    word_type = None
    try:
        url = f"https://en.bab.la/dictionary/finnish-english/{word}"
        headers = {'User-Agent': random.choice(user_agents)}
        response = requests.get(url, headers=headers)

        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            div_rows = soup.find("div", {"class": "wordPair"})

            if div_rows:
                first_span = soup.find_next("span", {"class": "flag fi"})

                if first_span:
                    second_span = soup.find_next("span")
                ...
                translation_span = soup.find("span", {"class": "babQuickResult"})
